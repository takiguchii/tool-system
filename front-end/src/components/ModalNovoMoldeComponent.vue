  ...
  <div class="fixed inset-0 bg-black/80 flex items-center justify-center z-50 animate-fade-in p-4 sm:p-6">
    <div class="bg-zinc-900 border border-zinc-700 rounded-xl w-full max-w-lg shadow-2xl overflow-hidden max-h-[95vh] flex flex-col">
      ...
      <div class="p-5 sm:p-6 overflow-y-auto flex-1 custom-scrollbar">
        <form @submit.prevent="salvarMolde" class="flex flex-col gap-5">
          ...
          <div class="grid grid-cols-2 gap-4">
            <div>
              ...
              <select v-model="novoMolde.empresaId" class="w-full bg-zinc-950 text-white border border-zinc-800 rounded-lg px-4 py-2.5 focus:outline-none focus:border-orange-500 transition-colors appearance-none cursor-pointer">
...
const salvarMolde = async () => {
  ...
  try {
    ...
    erro.value = e.response?.data || 'Erro ao salvar. Verifique se o código já existe ou se as imagens são válidas.'
  } finally {
