<template>
  <div class="bg-zinc-950 border border-zinc-800 rounded-xl p-4 md:p-8 shadow-2xl text-white relative">
    <div class="flex flex-col md:flex-row justify-between md:items-center mb-6 gap-4">
      <h2 class="text-2xl font-bold text-orange-500">Categorias</h2>
      <button @click="abrirModalNovo()" class="w-full md:w-auto bg-orange-500 hover:bg-orange-600 text-white font-bold py-2 px-4 rounded-lg">
        + Nova Categoria
      </button>
    </div>
    ...
    <div v-if="mostrarModal" class="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
      <div class="bg-zinc-900 border border-zinc-700 rounded-xl p-8 w-full max-w-md shadow-2xl">
        <h2 class="text-2xl font-bold text-white mb-6">{{ itemSelecionado.id ? 'Editar' : 'Nova' }} Categoria</h2>
        <form @submit.prevent="salvarItem">
          ...
          <div class="flex justify-end gap-3 mt-6">
            ...
            <button type="submit" class="bg-orange-500 hover:bg-orange-600 text-white font-bold px-6 py-2 rounded-lg">Salvar</button>
          </div>
        </form>
      </div>
    </div>
...
const buscarItens = async () => {
  ...
  try {
    ...
    erro.value = 'Falha ao buscar categorias.'
  } finally {
