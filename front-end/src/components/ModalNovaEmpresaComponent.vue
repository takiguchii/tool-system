<template>
  <div class="fixed inset-0 bg-black/80 flex items-center justify-center z-50 animate-fade-in p-4">
    <div class="bg-zinc-900 border border-zinc-700 rounded-xl p-8 w-full max-w-md shadow-2xl relative">
      <h2 class="text-2xl font-bold text-white mb-6">Cadastrar Empresa</h2>
      ...
      <form @submit.prevent="salvarEmpresa" class="flex flex-col gap-4">
        <div>
          ...
          <input type="text" v-model="novaEmpresa.nome" class="w-full bg-zinc-950 text-white border border-zinc-800 rounded-lg px-4 py-2 focus:outline-none focus:border-orange-500 focus:ring-1 focus:ring-orange-500" required placeholder="Ex: Fundição Domínio">
        </div>
...
const emit = defineEmits(['fechar', 'empresaCadastrada'])
...
const salvarEmpresa = async () => {
  ...
  try {
    ...
    await axios.post('/api/Empresa', novaEmpresa.value, {
      headers: { Authorization: `Bearer ${token}` }
    })
    emit('empresaCadastrada')
  } catch (e) {
